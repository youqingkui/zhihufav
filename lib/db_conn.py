...
class CollectionQueue(Base):
    ...
    fav_id = Column(Integer, default=0)
    note_guid = Column(String(64), default='')

    def __str__(self):
        return "CollectionQueue => { \
            cq_id:%d, answer_id:%d, api_url:'%s', web_url:'%s', title:'%s',  \
            is_collected:%d, add_time:%d, collected_time:%d, fav_id:%d, note_guid:'%s'}" % (
            self.cq_id, self.answer_id, self.api_url, self.web_url, self.title,
            self.is_collected, self.add_time, self.collected_time, self.fav_id, self.note_guid)
